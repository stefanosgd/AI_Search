...
class Genetic:
    def __init__(self, starting_city, cities, size):
        ...
        # print("Normalised fitness =", np.sum(self.fitness))
    ...
    def next_generation(self):
        ...
        for i in range(self.loops):
            order, index = self.pick_one()
            new_population[i], new_length[i] = self.mutate(order, index)
    ...
    def pick_one(self):
        ...
        while r > 0 and index < self.loops:
            r = r - self.fitness[index]
            index += 1
        index -= 1
        return self.population[index], index

    def mutate(self, order, mutated):
        mutate_attempt = list(order)
        ...
        mutate_length = self.mutated_distance(mutated, order, mutate_attempt, i, l)
